...
################# Brazil Wave 2 Dataset #####################

br_wave_2 <-  unlist(read.csv('Br_Wave_2.csv'))

# Visualizing the data
ggplot(as_tibble(br_wave_2), aes(value)) +
  geom_histogram(bins = 30, color = 'black', fill = 'grey') +
  scale_x_continuous('Cases') +
  scale_y_continuous('Frequency') +
  labs(title = "Brazil - Second Wave")

# 5 data summary 
summary(br_wave_2)
var(br_wave_2)
(d <- describe(br_wave_2))
(coef <- d$sd/d$mean)
...
# The second wave: from 11/2020 to 10/2021  
date <- dmy('1/11/2020')

as_tibble(br_wave_2) %>%
  mutate(days = seq(1,length(br_wave_2)), date = date + days(days)) %>%
  arrange(desc(value)) %>%
  View()
...
as_tibble(br_wave_2) %>%
  ...
  geom_line()+
  labs(title = "Brazil - Second Wave", y = 'Number of Cases', x = 'Date')

# What happened on the days with high death rate?
# There were three public holidays in this month
# 1- Apr 2nd, good friday (national holiday)
# 2- Apr 4th, easter (Observance)
# 4- Apr 21st, tiradentes day (national holiday)
...
Exp <- fitdistr(br_wave_2, "exponential")
...
ggplot(as_tibble(br_wave_2), aes(value)) +
  ...
  geom_smooth(aes(br_wave_2,dexp(br_wave_2,est_lambda)),se = F, color = 'red') +
  scale_x_continuous('Cases') +
...
ggplot(as_tibble(br_wave_2), aes(value)) +
  geom_smooth(aes(br_wave_2,ha_exponential(br_wave_2,dexp,pexp,est_lambda)))+
...
fit_ee <- vglm(value ~ 1, fam = expexpff, data = as_tibble(br_wave_2), trace = TRUE, maxit = 99)
...
# Exponentiated exponential distribution
eed <-  lamb_hat*alph_hat * (1 - exp(-1 * lamb_hat * br_wave_2))^(alph_hat - 1) * exp(-1 * lamb_hat * br_wave_2)

ggplot(as_tibble(br_wave_2), aes(value)) +
  geom_histogram(aes(y = ..density..), bins = 30, color = 'black', fill = 'grey') +
  geom_smooth(aes(br_wave_2,eed),se = F, color = 'orange') +
  scale_x_continuous('Cases') +
  scale_y_continuous('Density') +
...
ggplot(as_tibble(br_wave_2), aes(value)) +
  geom_smooth(aes(br_wave_2,ha_ee(br_wave_2,eed,FEE(br_wave_2,alph_hat,lamb_hat),lamb_hat,alph_hat)))+
  scale_y_continuous('h(t)')+
  scale_x_continuous('t') +
...
fit <- vglm(value ~ 1, fam = gammaR, data = as_tibble(br_wave_2), trace = TRUE)
...
ggplot(as_tibble(br_wave_2), aes(value)) +
  geom_histogram(aes(y = ..density..), bins = 30, color = 'black', fill = 'grey') +
  geom_smooth(aes(br_wave_2,dgamma(br_wave_2,a,b)), color = 'green', se = F) +
  scale_x_continuous('Cases') +
  scale_y_continuous('Density') +
...
ggplot(as_tibble(br_wave_2), aes(value)) +
  geom_smooth(aes(br_wave_2,ha_3(br_wave_2,dgamma,pgamma,a,b)))+
  scale_y_continuous('h(t)')+
  scale_x_continuous('t')
...
fit_w <- vglm(value ~ 1, fam = weibullRff, data = as_tibble(br_wave_2), trace = TRUE, maxit = 99)
k <- Coef(fit_w)[2]
l <- Coef(fit_w)[1]
l_w <-  logLik(fit_w)
...
ggplot(as_tibble(br_wave_2), aes(value)) +
  geom_histogram(aes(y = ..density..), bins = 30, color = 'black', fill = 'grey') +
  geom_smooth(aes(br_wave_2,dweibull(br_wave_2,k,l)),se = F, color = 'blue') +
  scale_x_continuous('Cases') +
  scale_y_continuous('Density') +
...
ggplot(as_tibble(br_wave_2), aes(value)) +
  geom_smooth(aes(br_wave_2,ha_weibull(br_wave_2,dweibull,pweibull,k,l)))+
  scale_y_continuous('h(t)')+
  scale_x_continuous('t')
...
fit_lg <- vglm(value ~ 1, fam = lognormal, data = as_tibble(br_wave_2), trace = TRUE, maxit = 99)
mu <- Coef(fit_lg)[1]
sd <- Coef(fit_lg)[2]
l_lg <-  logLik(fit_lg)
...
ggplot(as_tibble(br_wave_2), aes(value)) +
  geom_histogram(aes(y = ..density..), bins = 30, color = 'black', fill = 'grey') +
  geom_smooth(aes(br_wave_2,dlnorm(br_wave_2,mu,sd)),se = F, color = 'yellow') +
  scale_x_continuous('Cases') +
  scale_y_continuous('Density') +
...
fit_moe <-  maxlogL(br_wave_2, dist = "dMOE", link = list(over = c("l","t"), fun = c("log_link","log_link")))
...
ggplot(as_tibble(br_wave_2), aes(value)) +
  geom_histogram(aes(y = ..density..), bins = 30, color = 'black', fill = 'grey') +
  geom_smooth(aes(br_wave_2,dMOE(br_wave_2,lae,te)),se = F, color = 'cyan') +
  scale_x_continuous('Cases') +
  scale_y_continuous('Density') +
...
ggplot(as_tibble(br_wave_2), aes(value)) +
  geom_smooth(aes(br_wave_2,ha_moe(br_wave_2,dMOE,FMOE,lae,te)))+
  scale_y_continuous('h(t)')+
  scale_x_continuous('t')
...
fit_moee <-  maxlogL(br_wave_2, dist = "dMOEE", link = list(over = c("a","l","t"), fun = c("log_link","log_link","log_link")))
...
ggplot(as_tibble(br_wave_2), aes(value)) +
  geom_histogram(aes(y = ..density..), bins = 30, color = 'black', fill = 'grey') +
  geom_smooth(aes(br_wave_2,dMOEE(br_wave_2,al,la,t)),se = F, color = 'purple') +
  scale_x_continuous('Cases') +
  scale_y_continuous('Density') +
...
ggplot(as_tibble(br_wave_2), aes(value)) +
  geom_smooth(aes(br_wave_2,ha_moee(br_wave_2,dMOEE,FMOEE,al,la,t)))+
  scale_y_continuous('h(t)')+
  scale_x_continuous('t') +
  labs(title = "Marshall Olkin EE Distribution")
...
fit_mow <-  maxlogL(br_wave_2, dist = "dMOW", link = list(over = c("a","l","t"), fun = c("log_link","log_link","log_link")))
...
ggplot(as_tibble(br_wave_2), aes(value)) +
  geom_histogram(aes(y = ..density..), bins = 30, color = 'black', fill = 'grey') +
  geom_smooth(aes(br_wave_2,dMOW(br_wave_2,alw,law,tw)),se = F, color = 'brown') +
  scale_x_continuous('Cases') +
  scale_y_continuous('Density') +
...
ggplot(as_tibble(br_wave_2), aes(value)) +
  geom_smooth(aes(br_wave_2,ha_mow(br_wave_2,dMOW,FMOW,alw,law,tw)))+
  scale_y_continuous('h(t)')+
  scale_x_continuous('t') +
  labs(title = "Marshall Olkin Weibull Distribution")
...
fit_mopee <-  maxlogL(br_wave_2, dist = "dMOPEE", link = list(over = c("a","b","l"), fun = c("log_link","log_link","log_link")))
...
ggplot(as_tibble(br_wave_2), aes(value)) +
  geom_histogram(aes(y = ..density..), bins = 30, color = 'black', fill = 'grey') +
  geom_smooth(aes(br_wave_2,dMOPEE(br_wave_2,alp,beta,lap)),se = F, color = 'purple') +
  scale_x_continuous('Cases') +
  scale_y_continuous('Density') +
...
ggplot(as_tibble(br_wave_2), aes(value)) +
  geom_smooth(aes(br_wave_2,ha_mow(br_wave_2,dMOPEE,FMOPEE,alp,beta,lap)))+
  scale_y_continuous('h(t)')+
  scale_x_continuous('t')
...
ggplot(as_tibble(br_wave_2), aes(value)) +
  geom_histogram(aes(y = ..density..), bins = 30, color = 'black', fill = 'grey') +
  geom_smooth(aes(br_wave_2,dexp(br_wave_2,est_lambda), color = 'Exponential'),se = F) +
  geom_smooth(aes(br_wave_2,eed, color = 'Exponentiated Exponential'),se = F) +
  geom_smooth(aes(br_wave_2,dgamma(br_wave_2,a,b), color = 'Gamma'), se = F) +
  geom_smooth(aes(br_wave_2,dweibull(br_wave_2,k,l), color = "Weibull"),se = F) +
  geom_smooth(aes(br_wave_2,dlnorm(br_wave_2,mu,sd), color = 'Lognormal'),se = F) +
  geom_smooth(aes(br_wave_2,dMOE(br_wave_2,lae,te), color = 'MOE'),se = F) +
  geom_smooth(aes(br_wave_2,dMOEE(br_wave_2,al,la,t), color = 'MOEE'),se = F) +
  geom_smooth(aes(br_wave_2,dMOW(br_wave_2,alw,law,tw), color = 'MOW'),se = F) +
  geom_smooth(aes(br_wave_2,dMOPEE(br_wave_2,alp,beta,lap), color = 'MOPEE'),se = F) +
  scale_x_continuous('Cases') +
  scale_y_continuous('Density') +
...
p1 <- ks.test(br_wave_2,"pexp",est_lambda)

FEE <- function(x,alp,lam){
  (1 - exp(-lam * x) )^alp
}

p2 <- ks.test(br_wave_2,"FEE",alph_hat,lamb_hat)
p3 <- ks.test(br_wave_2,"pgamma",a,b)
p4 <- ks.test(br_wave_2,"pweibull",k,l)
p5 <- ks.test(br_wave_2,"plnorm",mu,sd)

FMOE <- function(x,l,t) {
  (1-exp(-l*x)) / (1- (1-t)*exp(-l*x))
}

p6 <-  ks.test(br_wave_2,"FMOE",lae,te)
...
p7 <-  ks.test(br_wave_2,"FMOEE",al,la,t)
...
p8 <-  ks.test(br_wave_2,"FMOW",alw,law,tw)
...
p9 <-  ks.test(br_wave_2,"FMOPEE",alp,beta,lap)
